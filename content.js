...
(function() {
  ...
  let popupTimeout;
  // --- REMOVED: isShortcutEnabled state is no longer needed
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    } else if (request.action === "show_and_focus_search_box") {
      showAndFocusSearchBox();
    }
  });

  function showAndFocusSearchBox() {
    // --- UPDATED: Shortcut is now always enabled ---
    if (!manualSearchBox) {
  ...
  // --- Definition Popup Logic (No changes here) ---
  ...
  // --- API Fetching and Formatting (No changes here) ---
  async function fetchDefinition(word) {
    try {
      const response = await fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${word}`);
      if (!response.ok) {
          return `<div class="error-title">Definition not found</div><p>Please check the word and try again.</p>`;
      }
  ...
  function createManualSearchBox() {
    ...
    // --- UPDATED: Simplified header with static shortcut label ---
    manualSearchBox.innerHTML = `
      ...
      <div class="shortcut-label">Shortcut: Alt+Shift+D</div>
    ...
    // --- REMOVED: Event listener for the toggle is no longer needed ---
